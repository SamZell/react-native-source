...
/**
 * 这个是 原生驱动js实现用户界面动画的主要实现类
 *它实现了一个动画节点图的管理接口，并实现了一个图形遍历每个动画帧的遍历算法。
 * 对于每一个动画帧，我们访问动画节点，可能已经更新，以及他们的子节点可能会使用父节点的价值观来更新自己。遍历算法的结束
 * 我们期望达到的一种特殊类型的节点：propsanimatednode，然后负责计算可以发送到本地视图层次结构的属性映射，以更新视图。
 * 这个类只能在主线程(即UI线程)访问
 *
 *
