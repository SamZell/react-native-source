...
/**
 * 催化本地模块的基类，它是写在java里，为了方便默认的实现{@link #initialize()}和{@link #onCatalystInstanceDestroy()}，
 * 子类继承之后 如果想覆盖这个方法，没必要去调用{@code super}
 ...
 *
 * BaseJavaModules可以跟碎片的生命周期事件关联着,当一个生命周期发生改变那么这个就把{@link CatalystInstance}它创建跟销毁
 ...
 *
 * Native方法可以通过{@link ReactMethod} 暴露给js端,这些方法只接受有限的参数类型
 * 原始：boolean int float double
 * String=>string
 * ReadableArray=>js array
 * ReadableMap=>js object
 * CallBack=>js function 只能做为最后一个参数，或是这会表达成功或失败的架设做为最后的2个参数
